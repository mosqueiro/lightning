...
/* zcored loves its backwards txids! */
static struct zcore_txid txid_from_hex(const char *hex)
{
	struct zcore_txid txid;

	if (!zcore_txid_from_hex(hex, strlen(hex), &txid))
		abort();
	return txid;
}
...
static struct secret secret_from_hex(const char *hex)
{
	 ...
	 *   (ZCore's convention for "compressed" private keys, i.e. keys
...
static void tx_must_be_eq(const struct zcore_tx *a,
			  const struct zcore_tx *b)
{
	u8 *lina, *linb;
	size_t i;
...
static void report_htlcs(const struct zcore_tx *tx,
			 ...
			 u32 feerate_per_kw)
{
	size_t i, n;
	struct zcore_txid txid;
	struct zcore_tx **htlc_tx;
	struct zcore_signature *remotehtlcsig;
	struct keyset keyset;
	u8 **wscript;

	htlc_tx = tal_arrz(tmpctx, struct zcore_tx *, tal_count(htlc_map));
	remotehtlcsig = tal_arr(tmpctx, struct zcore_signature,
				tal_count(htlc_map));
	...
	for (i = 0; i < tal_count(htlc_map); i++) {
		...
		if (htlc_owner(htlc) == LOCAL) {
			htlc_tx[i] = htlc_timeout_tx(htlc_tx, tx->chainparams,
						     ...
						     feerate_per_kw,
						     &keyset);
			wscript[i] = zcore_wscript_htlc_offer(tmpctx,
								local_htlckey,
								remote_htlckey,
								&htlc->rhash,
			...
			htlc_tx[i] = htlc_success_tx(htlc_tx, tx->chainparams,
						     ...
						     feerate_per_kw,
						     &keyset);
			wscript[i] = zcore_wscript_htlc_receive(tmpctx,
								  &htlc->expiry,
								  local_htlckey,
								  remote_htlckey,
								  &htlc->rhash,
	...
	for (i = 0; i < tal_count(htlc_map); i++) {
		struct zcore_signature localhtlcsig;
		const struct htlc *htlc = htlc_map[i];
...
static void report(struct zcore_tx *tx,
		   const u8 *wscript,
...
{
	char *txhex;
	struct zcore_signature localsig, remotesig;
	u8 **witness;
	...
	sign_tx_input(tx, 0,
		      ...
		      &localsig);
	printf("# local_signature = %s\n",
	       type_to_string(tmpctx, struct zcore_signature, &localsig));

	witness =
	    zcore_witness_2of2(tx, &localsig, &remotesig,
...
#ifdef DEBUG
static struct amount_sat calc_fee(const struct zcore_tx *tx, struct amount_sat input)
{
	size_t i;
...
int main(void)
{
	 ...
	 *  - Private keys are displayed as 32 bytes plus a trailing 1
	 *    (ZCore's convention for "compressed" private keys, i.e. keys for
	...
	wscript = zcore_redeem_2of2(tmpctx, &local_funding_pubkey,
