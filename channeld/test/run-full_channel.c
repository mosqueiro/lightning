...
static struct zcore_tx *tx_from_hex(const tal_t *ctx, const char *hex)
{
	return zcore_tx_from_hex(ctx, hex, strlen(hex));
}
...
static void txs_must_be_eq(struct zcore_tx **a, struct zcore_tx **b)
{
	size_t i;
...
int main(void)
{
	...
	struct zcore_tx *raw_tx, **txs, **txs2;
	...
	*/
	...
	raw_tx->chainparams = chainparams_for_network("zcore");
	zcore_tx_input_set_witness(raw_tx, 0, NULL);
