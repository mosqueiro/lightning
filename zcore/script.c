...
/* ZCore's OP_HASH160 is RIPEMD(SHA256()) */
...
static void add_push_sig(u8 **scriptp, const struct zcore_signature *sig)
{
	u8 der[73];
...
/* ZCore wants DER encoding. */
static u8 *stack_sig(const tal_t *ctx, const struct zcore_signature *sig)
{
	u8 der[73];
	size_t len = signature_to_der(der, sig);
...
/* ZCore script stack values are a special, special snowflake.
 *
...
u8 *zcore_redeem_2of2(const tal_t *ctx,
...
u8 *scriptpubkey_p2sh_hash(const tal_t *ctx, const struct ripemd160 *redeemhash)
{
	...
	assert(tal_count(script) == ZCORE_SCRIPTPUBKEY_P2SH_LEN);
	return script;
}
...
u8 *scriptpubkey_p2pkh(const tal_t *ctx, const struct zcore_address *addr)
{
	u8 *script = tal_arr(ctx, u8, 0);
	...
	assert(tal_count(script) == ZCORE_SCRIPTPUBKEY_P2PKH_LEN);
	return script;
}
...
u8 *zcore_redeem_p2pkh(const tal_t *ctx, const struct pubkey *pubkey,
...
u8 *zcore_redeem_p2sh_p2wpkh(const tal_t *ctx, const struct pubkey *key)
{
	...
	assert(tal_count(script) == ZCORE_SCRIPTPUBKEY_P2WPKH_LEN);
	return script;
}

u8 *zcore_scriptsig_p2sh_p2wpkh(const tal_t *ctx, const struct pubkey *key)
{
	u8 *redeemscript = zcore_redeem_p2sh_p2wpkh(ctx, key), *script;
...
u8 **zcore_witness_p2wpkh(const tal_t *ctx,
			    const struct zcore_signature *sig,
...
u8 *scriptpubkey_p2wsh(const tal_t *ctx, const u8 *witnessscript)
{
	...
	assert(tal_count(script) == ZCORE_SCRIPTPUBKEY_P2WSH_LEN);
	return script;
}
...
u8 **zcore_witness_2of2(const tal_t *ctx,
			  const struct zcore_signature *sig1,
			  const struct zcore_signature *sig2,
			  const struct pubkey *key1,
			  const struct pubkey *key2)
{
	...
	witness[3] = zcore_redeem_2of2(witness, key1, key2);
	return witness;
}
...
bool is_p2pkh(const u8 *script, struct zcore_address *addr)
{
	size_t script_len = tal_count(script);

	if (script_len != ZCORE_SCRIPTPUBKEY_P2PKH_LEN)
		return false;
	if (script[0] != OP_DUP)
		return false;
...
bool is_p2sh(const u8 *script, struct ripemd160 *addr)
{
	...
	if (script_len != ZCORE_SCRIPTPUBKEY_P2SH_LEN)
		return false;
...
bool is_p2wsh(const u8 *script, struct sha256 *addr)
{
	...
	if (script_len != ZCORE_SCRIPTPUBKEY_P2WSH_LEN)
		return false;
	if (script[0] != OP_0)
		return false;
...
bool is_p2wpkh(const u8 *script, struct zcore_address *addr)
{
	size_t script_len = tal_count(script);

	if (script_len != ZCORE_SCRIPTPUBKEY_P2WPKH_LEN)
		return false;
	if (script[0] != OP_0)
		return false;
...
u8 **zcore_witness_sig_and_element(const tal_t *ctx,
				     const struct zcore_signature *sig,
 ...
 *     OP_CHECKSIG
 */
u8 *zcore_wscript_to_local(const tal_t *ctx, u16 to_self_delay,
 ...
 *     OP_ENDIF
 */
u8 *zcore_wscript_htlc_offer_ripemd160(const tal_t *ctx,
...
u8 *zcore_wscript_htlc_offer(const tal_t *ctx,
			       const struct pubkey *localhtlckey,
			       const struct pubkey *remotehtlckey,
			       ...
			       const struct pubkey *revocationkey)
{
	...
	return zcore_wscript_htlc_offer_ripemd160(ctx, localhtlckey,
						    remotehtlckey,
 ...
 *     OP_ENDIF
 */
u8 *zcore_wscript_htlc_receive_ripemd(const tal_t *ctx,
...
u8 *zcore_wscript_htlc_receive(const tal_t *ctx,
				 const struct abs_locktime *htlc_abstimeout,
				 const struct pubkey *localhtlckey,
				 const struct pubkey *remotehtlckey,
				 const struct sha256 *payment_hash,
				 const struct pubkey *revocationkey)
{
	...
	ripemd160(&ripemd, payment_hash->u.u8, sizeof(payment_hash->u));
	return zcore_wscript_htlc_receive_ripemd(ctx, htlc_abstimeout,
 ...
 */
u8 **zcore_witness_htlc_timeout_tx(const tal_t *ctx,
...
u8 **zcore_witness_htlc_success_tx(const tal_t *ctx,
				     const struct zcore_signature *localhtlcsig,
...
u8 *zcore_wscript_htlc_tx(const tal_t *ctx,
			    u16 to_self_delay,
			    const struct pubkey *revocation_pubkey,
			    const struct pubkey *local_delayedkey)
{
