#ifndef LIGHTNING_ZCORE_FEERATE_H
#define LIGHTNING_ZCORE_FEERATE_H
#include "config.h"
#include <ccan/build_assert/build_assert.h>
#include <ccan/short_types/short_types.h>

/* zcored considers 250 satoshi per kw to be the minimum acceptable fee:
 * less than this won't even relay.
 */
#define ZCORED_MINRELAYTXFEE_PER_KW 250
/*
 * But zcored uses vbytes (ie. (weight + 3) / 4) for this
 * calculation, rather than weight, meaning we can disagree since we do
 * it sanely (as specified in BOLT #3).
 */
#define FEERATE_ZCORED_SEES(feerate, weight) \
	(((feerate) * (weight)) / 1000 * 1000 / ((weight) + 3))
/* ie. fee = (feerate * weight) // 1000
 * zcored needs (worst-case): fee * 1000 / (weight + 3) >= 250
 *
...
{
	/* Assert that zcored will see this as above minRelayTxFee */
	BUILD_ASSERT(FEERATE_ZCORED_SEES(FEERATE_FLOOR, MINIMUM_TX_WEIGHT)
		     >= ZCORED_MINRELAYTXFEE_PER_KW);
	/* And a lesser value won't do */
	BUILD_ASSERT(FEERATE_ZCORED_SEES(FEERATE_FLOOR-1, MINIMUM_TX_WEIGHT)
		     < ZCORED_MINRELAYTXFEE_PER_KW);
	/* And I'm right about it being OK for larger txs, too */
	BUILD_ASSERT(FEERATE_ZCORED_SEES(FEERATE_FLOOR, (MINIMUM_TX_WEIGHT*2))
		     >= ZCORED_MINRELAYTXFEE_PER_KW);

	return FEERATE_FLOOR;
}
#endif /* LIGHTNING_ZCORE_FEERATE_H */
