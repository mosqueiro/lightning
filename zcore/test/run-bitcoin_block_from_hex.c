...
int main(void)
{
	struct zcore_blkid prev;
	struct sha256_double merkle;
	struct zcore_txid txid, expected_txid;
	struct zcore_block *b;

	setup_locale();
	chainparams = chainparams_for_network("zcore");
	b = zcore_block_from_hex(NULL, chainparams,
				   block, strlen(block));

	assert(b);
	assert(b->hdr.version == CPU_TO_LE32(0x6592a000));
	zcore_blkid_from_hex("0000000000000f31173e973bc00e452b1fac350066df7db2adec1e3224ea5bc1", strlen("0000000000000f31173e973bc00e452b1fac350066df7db2adec1e3224ea5bc1"), &prev);
	assert(zcore_blkid_eq(&prev, &b->hdr.prev_hash));
	...
	zcore_txid(b->tx[0], &txid);
	zcore_txid_from_hex("14d86acd2158acd1f59ab77ab251e3f5073db905a7b2aed25d3ba7780c3d790c",
			      strlen("14d86acd2158acd1f59ab77ab251e3f5073db905a7b2aed25d3ba7780c3d790c"),
			      &expected_txid);
	assert(zcore_txid_eq(&txid, &expected_txid));

	zcore_txid(b->tx[1], &txid);
	zcore_txid_from_hex("c261a53121cc9841f843e2e6e0cff337e4f3c5eee788c982a0bffe771ce69919",
			      strlen("c261a53121cc9841f843e2e6e0cff337e4f3c5eee788c982a0bffe771ce69919"),
			      &expected_txid);
	assert(zcore_txid_eq(&txid, &expected_txid));

	zcore_txid(b->tx[2], &txid);
	zcore_txid_from_hex("80cea306607b708a03a1854520729da884e4317b7b51f3d4a622f88176f5e034",
