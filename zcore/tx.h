#ifndef LIGHTNING_ZCORE_TX_H
#define LIGHTNING_ZCORE_TX_H
#include "config.h"
#include "shadouble.h"
#include "signature.h"
#include "varint.h"
...
#define ZCORE_TX_DEFAULT_SEQUENCE 0xFFFFFFFF

struct zcore_txid {
	struct sha256_double shad;
};
/* Define zcore_txid_eq */
STRUCTEQ_DEF(zcore_txid, 0, shad.sha.u);
 ...
 */
const u8 *zcore_tx_output_get_script(const tal_t *ctx, const struct zcore_tx *tx, int outnum);
...
/**
 * Wrap the raw txhash in the wally_tx_input into a zcore_txid
 */
 ...
 *
 * Mainly for the transition from `zcore_tx` to the `wally_tx`. Checks that
