...
static void dump_tx(const char *msg,
		    const struct zcore_tx *tx, size_t inputnum,
		    const u8 *script,
...
static void dump_tx(const char *msg UNUSED,
		    const struct zcore_tx *tx UNUSED, size_t inputnum UNUSED,
...
/* Taken from https://github.com/zcore/zcore/blob/master/src/key.cpp */
...
static void zcore_tx_hash_for_sig(const struct zcore_tx *tx, unsigned int in,
				    const u8 *script,
		   ...
		   struct zcore_signature *sig)
{
	...
	zcore_tx_hash_for_sig(tx, in, script, sighash_type, &hash);
		  ...
		  const struct zcore_signature *sig)
{
	...
	zcore_tx_hash_for_sig(tx, input_num, script, sig->sighash_type, &hash);
...
/* Stolen direct from zcore/src/script/sign.cpp:
// Copyright (c) 2009-2010 Satoshi Nakamoto
// Copyright (c) 2009-2014 The ZCore Core developers
...
static char *zcore_signature_to_hexstr(const tal_t *ctx,
					 const struct zcore_signature *sig)
{
	u8 der[73];
...
REGISTER_TYPE_TO_STRING(zcore_signature, zcore_signature_to_hexstr);
