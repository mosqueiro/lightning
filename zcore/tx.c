...
int zcore_tx_add_output(struct zcore_tx *tx, const u8 *script,
			  struct amount_sat amount)
{
	...
	zcore_tx_output_set_amount(tx, i, amount);

	return i;
}

int zcore_tx_add_multi_outputs(struct zcore_tx *tx,
				 struct zcore_tx_output **outputs)
{
	for (size_t j = 0; j < tal_count(outputs); j++)
		zcore_tx_add_output(tx, outputs[j]->script,
				      outputs[j]->amount);

	return tx->wtx->num_outputs;
}

static bool elements_tx_output_is_fee(const struct zcore_tx *tx, int outnum)
{
 ...
 */
static struct amount_sat zcore_tx_compute_fee(const struct zcore_tx *tx)
{
...
int elements_tx_add_fee_output(struct zcore_tx *tx)
{
	struct amount_sat fee = zcore_tx_compute_fee(tx);
	int pos = -1;
	...
	if (pos == -1) {
		return zcore_tx_add_output(tx, NULL, fee);
	} else {
		zcore_tx_output_set_amount(tx, pos, fee);
		return pos;
	}
}

int zcore_tx_add_input(struct zcore_tx *tx, const struct zcore_txid *txid,
			 ...
			 struct amount_sat amount, u8 *script)
{
	...
	wally_tx_input_init_alloc(txid->shad.sha.u.u8,
				  sizeof(struct zcore_txid), outnum, sequence,
...
bool zcore_tx_check(const struct zcore_tx *tx)
{
	u8 *newtx;
	size_t written;
...
void zcore_tx_output_set_amount(struct zcore_tx *tx, int outnum,
				  struct amount_sat amount)
{
	u64 satoshis = amount.satoshis; /* Raw: low-level helper */
...
const u8 *zcore_tx_output_get_script(const tal_t *ctx,
				       const struct zcore_tx *tx, int outnum)
{
...
struct amount_asset zcore_tx_output_get_amount(const struct zcore_tx *tx,
						 int outnum)
{
...
void zcore_tx_input_set_witness(struct zcore_tx *tx, int innum,
				  u8 **witness)
{
...
void zcore_tx_input_set_script(struct zcore_tx *tx, int innum, u8 *script)
{
	wally_tx_set_input_script(tx->wtx, innum, script, tal_bytelen(script));
}

const u8 *zcore_tx_input_get_witness(const tal_t *ctx,
				       const struct zcore_tx *tx, int innum,
				       int witnum)
{
...
void zcore_tx_input_get_txid(const struct zcore_tx *tx, int innum,
			       struct zcore_txid *out)
{
	assert(innum < tx->wtx->num_inputs);
	assert(sizeof(struct zcore_txid) ==
	       sizeof(tx->wtx->inputs[innum].txhash));
	memcpy(out, tx->wtx->inputs[innum].txhash, sizeof(struct zcore_txid));
}

/* BIP144:
 * If the witness is empty, the old serialization format should be used. */
static bool uses_witness(const struct zcore_tx *tx)
{
	size_t i;
 ...
 */
static void push_tx(const struct zcore_tx *tx,
...
u8 *linearize_tx(const tal_t *ctx, const struct zcore_tx *tx)
{
...
size_t zcore_tx_weight(const struct zcore_tx *tx)
{
	size_t weight;
...
void zcore_txid(const struct zcore_tx *tx, struct zcore_txid *txid)
{
...
/* Use the zcore_tx destructor to also free the wally_tx */
static void zcore_tx_destroy(struct zcore_tx *tx)
{
	wally_tx_free(tx->wtx);
}

struct zcore_tx *zcore_tx(const tal_t *ctx,
			      const struct chainparams *chainparams,
			      varint_t input_count, varint_t output_count)
{
	struct zcore_tx *tx = tal(ctx, struct zcore_tx);
	assert(chainparams);
	...
	wally_tx_init_alloc(WALLY_TX_VERSION_2, 0, input_count, output_count,
			    &tx->wtx);
	tal_add_destructor(tx, zcore_tx_destroy);
...
struct zcore_tx *pull_zcore_tx(const tal_t *ctx, const u8 **cursor,
				   size_t *max)
{
	size_t wsize;
	int flags = WALLY_TX_FLAG_USE_WITNESS;
...
struct zcore_tx *zcore_tx_from_hex(const tal_t *ctx, const char *hex,
				       size_t hexlen)
{
	const char *end;
	u8 *linear_tx;
...
/* <sigh>.  ZCored represents hashes as little-endian for RPC. */
...
bool zcore_txid_from_hex(const char *hexstr, size_t hexstr_len,
...
bool zcore_txid_to_hex(const struct zcore_txid *txid,
...
static char *fmt_zcore_tx(const tal_t *ctx, const struct zcore_tx *tx)
{
...
static char *fmt_zcore_txid(const tal_t *ctx, const struct zcore_txid *txid)
{
	char *hexstr = tal_arr(ctx, char, hex_str_size(sizeof(*txid)));

	zcore_txid_to_hex(txid, hexstr, hex_str_size(sizeof(*txid)));
	return hexstr;
}

REGISTER_TYPE_TO_STRING(zcore_tx, fmt_zcore_tx);
REGISTER_TYPE_TO_STRING(zcore_txid, fmt_zcore_txid);
